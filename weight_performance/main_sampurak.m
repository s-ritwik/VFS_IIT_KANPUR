...
for k=1:size(V_cruise,2)
    ...
    %optimisation
                ...
                while k_min <= k_max
                    ...
                    [thrust_1, power_1, torque_1, theta_1, err_1, FM, BL, mech_power,CP,CT] = ...
                        Rotor_opt(R(i), c(i,j,k), thetatw, RPM(j), Nb, 3, GW(n), trans_loss, nondp, motor_efficiency, nondt, theta0(k_mid), electrical_loss, rho,N_rotors);
                        %2 is for airfoil
    ...
    %% -----------------------------Cruise performance of the rotor--------------------------------
                ...
                while k_min2 <= k_max2
                    ...
                    omega2(i,j,k)=k_mid2*2*pi/60;
                    nondt2=rho*A*(omega2(i,j,k)*R(i))^2;%non-dimentional making terms for thrust
                    nondp2=rho*A*(omega2(i,j,k)*R(i))^3;%non-dimentional making terms for pressure
                    % for cruise fixed wing
                    [thrust_2, power_2, torque_2, theta_2, err_2, FM2, BL2, mech_power2,CP2,CT2] = ...
                        Rotor_opt(R(i), c(i,j,k), thetatw, k_mid2, Nb, 3, GW(n)/L_by_D/N_rotors_cruise, trans_loss, nondp2, motor_efficiency, nondt2, theta_h(i,j,k), electrical_loss, rho,1);
                    if err_2 > 5
                        k_max2 = k_mid2 - 1;
                        err2(i,j,k)= err_2;
                        Cruise_RPM(i,j,k)=k_mid2;
                        power_cruise_hover(i,j,k)=power_2;% 0.85 is aerodynamic efficiency
                    else
                        k_min2 = k_mid2 + 1;
    ...
    %-----------Total power and thrust in hover--------------------------------------------------------------------------- 
                ...
                % print("1");
                Thrust_cruise(i,j,k)=GW(n)/L_by_D*g;
    ...
    %-----------Section 5 : 9deg climb--------------------------------------------------------------
                ...
                %where L=Wcos(9)
                try
                    power_5(i,j,k)=power_cruise(i,j,k)+ V_cruise_climb*GW(n)*g; % by using excess power formula
                    % power_5(i,j,k)= power_5(i,j,k)*N_rotors_cruise;
                catch
    ...
    %           here to hover for max T9 seconds.
                ...
                eta = thrust_h(i,j,k)*V_endu_cruise/(P_cruise_endurance(i,j,k)*L_by_D);
    ...
    %% -------------------- ENERGY CALCULATIONS------------------------------------------------------------------
                % Total Energy
                total_energy(i,j,k) = e_section_2(i,j,k) + e_section_3(i,j,k) + e_section_4(i,j,k) + e_section_5(i,j,k) + e_section_6(i,j,k) + e_section_7(i,j,k) + e_section_8(i,j,k) +...
                 e_section_9(i,j,k) + e_section_10(i,j,k) + e_section_11(i,j,k) + e_section_12(i,j,k) + e_section_13(i,j,k) + e_section_14(i,j,k) + e_section_15(i,j,k);   
                ...
                % Hydrogen powered
                energy_init(i,j,k)=e_section_2(i,j,k) + e_section_5(i,j,k) + ...
                  e_section_6(i,j,k) + e_section_7(i,j,k) +  e_section_9(i,j,k) + ...
                  e_section_10(i,j,k) + e_section_11(i,j,k) + e_section_12(i,j,k) ;
    ...
    %-----------HYDROGEN FUEL CELL-----------------------------------------------------------------------------------------------------%
                ...
                % mfuel_cell(i,j,k) = total_energy/(1000*3600*0.348061)
    ...
    %-----------NEW GROSS WEIGHT----------------------------------------------------------------------------------------
                mgross(i,j,k) = mempty+mfuel_cell(i,j,k)+mpayload ;%new gross weight
                m=m+1;
                n=n+1;
                GW(n) = mgross(i,j,k);
